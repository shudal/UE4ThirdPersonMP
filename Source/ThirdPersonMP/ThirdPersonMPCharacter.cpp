...
void AThirdPersonMPCharacter::OnHealthUpdate()
{
	...
	{
		//GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Blue, TEXT("is local cotroll"));
		UE_LOG(LogClass, Log, TEXT("is local cotroll"));
		FString healthMessage = FString::Printf(TEXT("You now have %f health remaining."), CurrentHealth);
		//GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Blue, healthMessage);
		UE_LOG(LogClass, Log, TEXT("%s"),*healthMessage);
		if (CurrentHealth <= 0)
		{
			FString deathMessage = FString::Printf(TEXT("You have been killed."));
			//GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, deathMessage);
			UE_LOG(LogClass, Log, TEXT("%s"),*deathMessage);
		}
	}
	else {
		UE_LOG(LogClass, Log, TEXT("not local control"));
		//GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Blue, TEXT("not local control"));
	}

	//服务器特定的功能
	if (GetWorld()->IsServer())
	{
		//GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Blue, TEXT("is server"));
		UE_LOG(LogClass, Log, TEXT("is server"));
		FString healthMessage = FString::Printf(TEXT("%s now has %f health remaining."), *GetFName().ToString(), CurrentHealth);
		//GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Blue, healthMessage);
		UE_LOG(LogClass, Log, TEXT("%s"), *healthMessage);
	}
	else {
		//GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Blue, TEXT("not server"));
		UE_LOG(LogClass, Log, TEXT("not server"));
	}
...
FString AThirdPersonMPCharacter::GetMyLog()
{
	return logS;
}
